...
import com.luck.picture.lib.adapter.PicturePreviewSelectAdapter;
...
public class PicturePreviewActivity extends PictureBaseActivity implements
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        original_image = (TextView) findViewById(R.id.originalImage);
        original_image.setOnClickListener(this);
        original_image.setSelected(!config.isCompress);
                ...
                if (images != null && images.size() > 0) {
                    ...
                    if (!TextUtils.isEmpty(pictureType)) {
                        boolean toEqual = PictureMimeType.
                                isAddFile(pictureType, image.getPictureType(), mContext, config);
                        if (!toEqual) {
//                            ToastManage.s(mContext,getString(R.string.picture_rule));
                    ...
                    if (isChecked) {
                        if(isLongVideo(image)){
                            //视频过长，点击后弹出提示，直接return，无法选择
                            check.setSelected(false);
                            return;
                        }
        ...
        viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int i) {
                ...
                picturePreviewSelectAdapter.setCurrentLocalMediaAndNotifyData(media);
                int p = selectImages.indexOf(media);
                if(p >= 0){
     ...
     * @param media
     */
    private boolean isLongVideo(LocalMedia media){
        int fileType = PictureMimeType.isPictureType(media.getPictureType());
        if(fileType == PictureMimeType.ofVideo()){
            if(media.getDuration() > PictureSelectorActivity.MAX_VIDEO_TIME){
                Toast.makeText(this, R.string.picture_video_to_long, Toast.LENGTH_SHORT).show();
                return true;
            }
        }
    ...
    private void initRecyclerViewAdapter(){
        ...
        picturePreviewSelectAdapter = new PicturePreviewSelectAdapter(this, selectImages);
        recyclerView.setAdapter(picturePreviewSelectAdapter);
        picturePreviewSelectAdapter.setItemClickListener(new PicturePreviewSelectAdapter.ItemClickListener() {
            @Override
            public void onClick(LocalMedia localMedia) {
                if(localMedia != null){
                    int index = images.indexOf(localMedia);
                    if(index >= 0){
        ...
        if(images != null){
            picturePreviewSelectAdapter.setCurrentLocalMediaAndNotifyData(images.get(position));
        }
    }
    ...
    public void onSelectNumChange(boolean isRefresh) {
        ...
        if (enable) {
            ...
            if (numComplete) {
                ...
                tv_ok.setBackgroundResource(R.drawable.btn_fillet_orange);
                tv_ok.setTextColor(0xffffffff);
                String done = getString(R.string.picture_done2, selectImages.size());
                if(config.buttonType == 1){
                    done = getString(R.string.picture_completed, selectImages.size());
                }
                tv_ok.setText(done);
            }
        } else {
            ...
            if (numComplete) {
                ...
                tv_ok.setBackgroundResource(R.drawable.btn_fillet_orange_disable);
                tv_ok.setTextColor(0x7fffffff);

                String done = getString(R.string.picture_done);
                if(config.buttonType == 1){
                    done = getString(R.string.picture_please_select);
                }
                tv_ok.setText(done);
            }
        }
    ...
    public void onClick(View view) {
        ...
        if(id == R.id.originalImage){
            //原图
            boolean isChecked = original_image.isSelected();
            original_image.setSelected(!isChecked);

            config.isCompress = isChecked;

            if (!isChecked) {
                if (animation != null) {
                    original_image.startAnimation(animation);
                }
            }
        }
    }
    ...
    private void imgClick(){
        if (isShow) {
            /*//初始化
            Animation animationTopHide = new TranslateAnimation(0f, 0f, 0f, -rl_title.getHeight());
            //设置动画时间
            animationTopHide.setDuration(500);
            animationTopHide.setFillAfter(true);
            rl_title.startAnimation(animationTopHide);*/

            float translationY = rl_title.getTranslationY();
            ObjectAnimator moveanim = ObjectAnimator.ofFloat(rl_title, "translationY", translationY, -rl_title.getHeight());
            moveanim.start();

            ObjectAnimator anim = ObjectAnimator.ofFloat(recyclerView, "alpha", 1f, 0.0f);
            anim.setDuration(500);// 动画持续时间
            anim.start();

            ObjectAnimator anim2 = ObjectAnimator.ofFloat(select_bar_layout, "alpha", 1.0f, 0.0f);
            anim2.setDuration(500);// 动画持续时间
            anim2.start();
        } else {
            float translationY = rl_title.getTranslationY();
            ObjectAnimator moveanim = ObjectAnimator.ofFloat(rl_title, "translationY", translationY, 0);
            moveanim.start();

            //属性动画
            ObjectAnimator anim = ObjectAnimator.ofFloat(recyclerView, "alpha", 0.0f, 1.0f);
            anim.setDuration(500);// 动画持续时间
            anim.start();

            ObjectAnimator anim2 = ObjectAnimator.ofFloat(select_bar_layout, "alpha", 0.0f, 1.0f);
            anim2.setDuration(500);// 动画持续时间
            anim2.start();
        }
