...
public final class PictureMimeType {
     ...
     * @return
     */
    public static boolean isAddFile(String p1, String p2, Context mContext, PictureSelectionConfig config) {
        switch (config.actionType){
            case 1:
                return action01(p1, p2, mContext);
            default:
                return true;
        }
//        return isPictureType(p1) == isPictureType(p2);
    ...
    private static boolean action01(String p1, String p2, Context mContext){
        if(isVideo(p1)){
            if(isVideo(p2)){
                //只能选择一个视频
                ToastManage.s(mContext, mContext.getString(R.string.picture_one_video));
            } else{
                //视频跟图片不能一起选择
                ToastManage.s(mContext, mContext.getString(R.string.picture_rule));
            }
            return false;
        } else{
            if(isVideo(p2)){
